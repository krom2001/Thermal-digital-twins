function [combinations, testrel, testabs] = combinationFinder(tempwindow, threshold, uniformcases, absnet, net, net2, net3, net4)
    ...
    %generate scores
    %order of the sensors is 2,1,3,4,5,6.
    ...
    for cell = 1:numel(effectiveradius)
        if effectiveradius(cell)>threshold
            ...
            inp = [score1, score2, score3, score4, score5, cell]';
            %predict relative radii
            relativeRadii(1:11, 4*numMinRadius-3) = net4(inp);
            [~, I1] = max(relativeRadii(1:11, 4*numMinRadius-3));
            relativeRadii(1:11, 4*numMinRadius-2) = net3(inp);
            [~, I2] = max(relativeRadii(1:11, 4*numMinRadius-2));
            relativeRadii(1:11, 4*numMinRadius-1) = net2(inp);
            [~, I3] = max(relativeRadii(1:11, 4*numMinRadius-1));
            relativeRadii(1:11, 4*numMinRadius) = net(inp);
