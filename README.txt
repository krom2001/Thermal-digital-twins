...
figures/		=> Figures generated by the code for the first data set.

figures(v2)/		=> Figures generated by the code for the second (v2) data set.
...
combinationFinder.m	=> A function which takes as arguments a set of temperature readings from the 6 nodes on a beam
			   ...
			   them with a confidence level for the prediction. The output comes in the same format as the 
			   output of the numtocomb.m function.
...
Make sure the files in 'save_net/' are for the data set you want to run (5_seg_beam_csv/ or 5_seg_beam_csv_v2/).
